...
export function Navbar() {
  ...
  const [modalType, setModalType] = useState<"prompt" | "info">("prompt");
  ...
  const handleAdminToggle = () => {
    if (isAdmin) {
      ...
      setModalConfig({ 
        title: "Admin Access", 
        description: "Enter password to enable production modifications" 
      });
      setIsModalOpen(true);
    }
  };

  const handleConfirm = (pass?: string) => {
    if (modalType === "prompt") {
      if (pass) {
        if (!unlock(pass)) {
          setModalType("info");
  ...
  return (
    <>
      ...
      <ThematicModal
        ...
        confirmText={modalType === "prompt" ? "Unlock" : "Okay"}
